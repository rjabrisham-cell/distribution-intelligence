...
def page(monkeypatch):
    ...
    saved = Mock(return_value=result)
    monkeypatch.setattr(routes, "saved_readiness", saved)
    ...
    return SimpleNamespace(db=db, matching=matching, audit=audit, result=result,
                           summary=summary, batch=batch, request=request, saved=saved, project=project)
...
def test_refresh_reads_snapshot_without_matching_audit_or_writes(page):
...
def test_explicit_matching_keeps_legacy_post_and_redirect(page, monkeypatch):
    ...
    page.db.info = {"trial_account_id": 7}
    page.project.trial_result_batch_id = None
    page.project.trial_state = "VALIDATED"
    page.db.flush.side_effect = None
    repository = Mock()
    repository.enqueue.return_value = SimpleNamespace(project_id=23, batch_id=24)
    monkeypatch.setattr(routes, "MatchingJobRepository", lambda _: repository)
    from app.services.entitlement_service import EntitlementService
    monkeypatch.setattr(EntitlementService, "assert_trial", lambda *_: None)
    response = asyncio.run(routes.process_readiness(request, 23, page.db))
    service.run_for_batch.assert_not_called()
    repository.enqueue.assert_called_once_with(7, 23, 24)
    page.audit.assert_not_called()
    assert response.status_code == 303
    assert response.headers["location"] == "/projects/23/matching/status"
...
def test_audit_error_does_not_fall_back_to_matching(page):
    page.saved.side_effect = ValueError("audit failed")
    response = routes.readiness_step(page.request, 23, page.db)
...
def test_real_audit_preserves_confirmed_master_and_selected_evidence(page, monkeypatch):
    ...
    # Prepare once, as the worker does; page views consume the saved payload.
    page.saved.return_value = AuditRunner(project_id=23, batch_id=24, db=page.db).run()
